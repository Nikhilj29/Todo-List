...
function Today() {
    ...
    async function getTodays() {
        ...
        try {
            const res = await axios.post("http://localhost:5000/sendTodo", { fdate });
            const data = res.data;
            // data.forEach((e) => {
            //     if ((dayjs(e.fdate).isBefore(tooday) && dayjs(e.tdate).isAfter(tooday)) || dayjs(e.fdate).isSame(today) || dayjs(e.tdate).isSame(today) || dayjs(e.tdate).isAfter(tooday)) {
            ...
            // });
            data.forEach(data => {
                addTodo(data.fdate, data.tdate, data.todo);
            });
    ...
    console.log(newArr, "NOO");
    async function handleSubmit(e) {
        e.preventDefault();
        const formatfDate = fdate.format("ddd MMM DD YYYY");
        const formattDate = tdate.format("ddd MMM DD YYYY");
        addTodo(formatfDate, formattDate, todo);
        setFDate(dayjs());
        setTDate(dayjs());
        setTodo("");
        let newdata = { todo, fdate: formatfDate, tdate: formattDate, repeat: select, completed: false };
        ...
        <>
                ...
                <div style={{ display: "flex", flexDirection: "column", width: "80%" }}>
                    <Card sx={{ height: "13rem" }}>
                        <div style={{ display: "flex", flexDirection: "row", margin: "1rem" }}>
                            ...
                            <h1 style={{ marginLeft: "1rem" }}> Today</h1>
                        </div>
                        <TextField
                            variant="outlined"
                            placeholder="Search..."
                    ...
                    {todos.length === 0 ? <h1 style={{ marginLeft: "2rem" }}>No Task for Today Take Chill!!</h1> : (
                        <ul>
            ...
            {isOpen && (
                ...
                >
                    ...
                    <Box sx={{ marginTop: 2 }}>
                        <Button onClick={handleToggle} sx={{ color: "#705C53" }}>Cancel</Button>
                        <Button onClick={handleSubmit} variant="contained" sx={{ marginLeft: 1, backgroundColor: "#705C53" }}>
                            Add
                        </Button>
                    </Box>
                </Box>
